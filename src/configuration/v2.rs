...
impl crate::configuration::OddBoxConfiguration<OddBoxV2Config> for OddBoxV2Config {
    ...
    fn to_string(&self) -> anyhow::Result<String> {
        if self.version != crate::configuration::OddBoxConfigVersion::V2 {
            panic!(
                "This is a bug in odd-box. The configuration version is not V2. This should not happen."
            );
        }
        ...
        let new_config = super::v2::OddBoxV2Config {
            ...
            hosted_process: Some(
                old_config
                    ...
                    .unwrap_or_default()
                    .into_iter()
                    .map(|x| super::v2::InProcessSiteConfig {
                        log_level: None,
                        enable_lets_encrypt: Some(false),
                        proc_id: ProcId::new(),
                        active_port: None,
                        ...
                        hints: match x.h2_hint {
                            Some(crate::configuration::v1::H2Hint::H2) => {
                                Some(vec![crate::configuration::v2::Hint::H2])
                            }
                            Some(crate::configuration::v1::H2Hint::H2C) => {
                                Some(vec![crate::configuration::v2::Hint::H2C])
                            }
                            None => None,
                        },
            ...
            remote_target: Some(
                old_config
                    ...
                    .unwrap_or_default()
                    .iter()
                    .map(|x| super::v2::RemoteSiteConfig {
                        keep_original_host_header: None,
                        enable_lets_encrypt: Some(false),
                        disable_tcp_tunnel_mode: x.disable_tcp_tunnel_mode,
                        capture_subdomains: x.capture_subdomains,
                        forward_subdomains: x.forward_subdomains,
                        backends: vec![super::v2::Backend {
                            hints: match x.h2_hint {
                                Some(crate::configuration::v1::H2Hint::H2) => {
                                    Some(vec![crate::configuration::v2::Hint::H2])
                                }
