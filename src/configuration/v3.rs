...
pub struct InProcessSiteConfig {
    ...
    #[serde(skip)]
    // <-- dont want to even read or write this to the config file, nor exposed in the api docs
...
impl PartialEq for RemoteSiteConfig {
    fn eq(&self, other: &Self) -> bool {
        self.host_name == other.host_name
            ...
            && compare_option_bool(
                self.keep_original_host_header,
                other.keep_original_host_header,
            )
...
fn filter_backend(backend: &Backend, filter: &BackendFilter) -> bool {
    ...
    match filter {
        ...
        BackendFilter::H2CPriorKnowledge => hints.iter().any(|h| **h == Hint::H2CPK),
        BackendFilter::H2C => hints.iter().any(|h| **h == Hint::H2C),
    }
}
...
impl InProcessSiteConfig {
    ...
    pub async fn next_backend(
        ...
        // - otherwise, 80
        let port = if let Some(p) = self.active_port {
            p
        } else {
            if let Some(p) = self.port {
                p
            } else {
                if self.https.unwrap_or_default() {
                    443
                } else {
                    80
                }
        ...
        let filtered_backends = backends
            .iter()
            .filter(|x| filter_backend(x, &backend_filter))
            .collect::<Vec<&Backend>>();

        if filtered_backends.len() == 1 {
            return Some(filtered_backends[0].clone());
        };
        if filtered_backends.len() == 0 {
            return None;
        };

        let current_req_count_for_target_host_name = { 0 };

        let selected_backend = filtered_backends.get(
            (current_req_count_for_target_host_name % (filtered_backends.len() as usize)) as usize,
        );
...
impl crate::configuration::OddBoxConfiguration<OddBoxV3Config> for OddBoxV3Config {
    fn write_to_disk(&self) -> anyhow::Result<()> {
        let current_path = if let Some(p) = &self.path {
            p
        } else {
            bail!(ConfigurationUpdateError::Bug(
                "No path found to the current configuration".into()
            ))
        };
    ...
    fn to_string(&self) -> anyhow::Result<String> {
        if self.version != V3VersionEnum::V3 {
            panic!(
                "This is a bug in odd-box. The configuration version is not V3. This should not happen."
            );
        }
        ...
        if let Some(remote_sites) = &self.remote_target {
            for site in remote_sites {
                ...
                let backend_strings = site
                    .backends
                    .iter()
                    .map(|b| {
                        let https = if let Some(true) = b.https {
                            format!("https = true, ")
                        } else {
                            format!("")
                        };
    ...
    fn example() -> OddBoxV3Config {
        OddBoxV3Config {
            ...
            env_vars: vec![
                ...
                EnvVar {
                    key: "another_key".into(),
                    value: "another_val".into(),
                },
            ],
            ...
            hosted_process: Some(vec![InProcessSiteConfig {
...
impl std::fmt::Display for ConfigurationUpdateError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            // },
            ConfigurationUpdateError::Bug(e) => f.write_fmt(format_args!(
                "Failed to save due to a bug in odd-box: {}",
                e
            )),
        }
    }
            ...
            hosted_process: Some(
                old_config
                    ...
                    .map(|x| {
                        ...
                        let new_hints = if new_hints.len() == 0 {
                            None
                        } else {
                            Some(new_hints)
                        };
                                ...
                                .map(|b| {
                                    let mut new_hints: Vec<Hint> = b
                                        ...
                                        .filter_map(|x| match x {
                                            &crate::configuration::v2::Hint::H2 => Some(Hint::H2),
                                            &crate::configuration::v2::Hint::H2C => Some(Hint::H2C),
                                            &crate::configuration::v2::Hint::H2CPK => {
                                                Some(Hint::H2CPK)
                                            }
                                            &crate::configuration::v2::Hint::NOH2 => None, // we dont support this anymore
                                        })
