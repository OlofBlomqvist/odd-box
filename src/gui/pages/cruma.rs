...
impl OddBoxGui {
    pub(in crate::gui) fn view_cruma_ingress(&self) -> Element<'_, Message> {
        ...
        let mode_header = text("Connection Mode")
            .font(Font::MONOSPACE)
            .size(text_size(14));
        ...
        if self.cruma_auth_mode == CrumaAuthMode::Authenticated {
            ...
            mode_col = mode_col.push(
                column![
                    text("Tunnel ID").font(Font::MONOSPACE).size(text_size(12)),
                    id_input,
                    text("Tunnel Key").font(Font::MONOSPACE).size(text_size(12)),
                    key_input,
                    save_button,
                ]
                .spacing(scaled(8.0))
        ...
        let mode_box = container(mode_col)
            .padding(scaled(16.0))
            .width(Length::Fill)
            .style(|theme: &Theme| iced::widget::container::Style {
                background: Some(self.surface_panel_bg(theme).into()),
